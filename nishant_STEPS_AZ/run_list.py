...
resultPath="/home/kabir/Project/tripartiteSynapse/results/STEPS_AZ/4May2020"#+'/'+sim_type
...
for v,isi,sim in product(VDCCs,isis,sims[0:1]):
    ...
    time,conc=D.load(sim,fname,return_original_time=True)
    ...
    r_sample=np.random.randint(0,conc.shape[1],old_seed_num(v))
    ...
    for val,freq in zip(r_sample,count):
        if freq>0:
            ...
            run_and_average(resultPath=resultPath,dir=folder,seeds=range(1,freq+1))
    #"""
    """        
    resample = 1000
    nAP = 2
