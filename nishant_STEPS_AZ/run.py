...
def runAZTrials(CaData, seed=False):
    vesData = []
    if seed:
        resAZ, vesRel, bReac = simAZ(CaData[0], CaData[1], sim, r, seed=seed)
    else:
        resAZ, vesRel, bReac = simAZ(CaData[0], CaData[1], sim, r)
...
def run_and_average(resultPath="./", dir='nVDCC_4_dVDCC_60_nAZ_10_RRP_5_ISI_20_nAP_10', seeds=range(1, 201)):
    ...
    bReacdata = np.hstack((np.array([CaData[0]]).T, bReac))
